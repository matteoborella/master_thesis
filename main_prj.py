...
for i in range(len(atom_section)):
	if i in range(len(headtail_section)):
		...
		head.append(headtail_section[0][i])
		tail.append(headtail_section[1][i])
	else:
		head.append('None')
		tail.append('None')

list_keys=['atom_id','atom_name','x_coor','y_coor','z_coor','atom_type','subst_id','subst_name','charge','head','tail']		
list_values=[atom_id,atom_name,x_coor,y_coor,z_coor,atom_type,subst_id,subst_name,charge,head,tail]
